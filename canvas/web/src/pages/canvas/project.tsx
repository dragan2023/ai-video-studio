...
import { NautilusConsole } from "@/components/canvas/nautilus-console";
import { NautilusAssetSaveDialog } from "@/components/canvas/nautilus-asset-save-dialog";
...
function InfiniteCanvasPage() {
    ...
    // 12a：待上传到 nautilus 共享素材库的画布媒体节点。
    const [nautilusAssetNodeId, setNautilusAssetNodeId] = useState<string | null>(null);
    ...
    const nautilusAssetNode = nautilusAssetNodeId ? nodeById.get(nautilusAssetNodeId) || null : null;
    ...
    return (
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            ...
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                <NautilusConsole nodes={nodes} setNodes={setNodes} />
                ...
                <CanvasNodeHoverToolbar
                    ...
                    onSaveToNautilus={(node) => setNautilusAssetNodeId(node.id)}
                ...
                <NautilusAssetSaveDialog
                    node={nautilusAssetNode}
                    open={Boolean(nautilusAssetNode)}
                    onClose={() => setNautilusAssetNodeId(null)}
                    onSaved={(nodeId, asset, roles) => {
                        setNodes((prev) =>
                            prev.map((node) =>
                                node.id === nodeId
                                    ? {
                                          ...
                                          metadata: {
                                              ...
                                              assetId: asset.id,
                                              nautilusKind: "asset",
                                              roles: asset.roles.length ? asset.roles : roles,
