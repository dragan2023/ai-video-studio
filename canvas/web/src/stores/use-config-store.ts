...
import { SUNO_MUSIC_SCRIPT } from "@/lib/audio-suno-script";
...
// 自用 fork 预置：默认渠道 = 米醋API（生图 gpt-image-2）。
// 视频/文本/音频留空：视频走 ComfyUI 工作流（M12 接 model-plugin 脚本 + 工作流选择器），文本后续接 nautilus planner。
// 注意：apiKey 硬编码进源码仅限本地自用；对外分发前应改为环境变量注入（VITE_MICU_API_KEY 等）。
// 默认渠道（米醋API）。密钥不硬编码：从 VITE_MICU_API_KEY 注入（.env.local），留空则启动后手动在画布「配置」中填写。
const FORK_DEFAULT_BASE_URL = import.meta.env.VITE_MICU_BASE_URL || "https://www.micuapi.ai/v1";
const FORK_DEFAULT_API_KEY = import.meta.env.VITE_MICU_API_KEY || "";

export const defaultConfig: AiConfig = {
    channelMode: "local",
    baseUrl: FORK_DEFAULT_BASE_URL,
    apiKey: FORK_DEFAULT_API_KEY,
    apiFormat: "openai",
    channels: [
        {
            id: "default",
        ...
        {
            // Suno 音频生成：baseUrl/apiKey 由 VITE_SUNO_BASE_URL / VITE_SUNO_API_KEY 注入（.env.local），
            // 或启动后在画布「配置 → 渠道 suno」中手动填写；当前留空。
            id: "suno",
            name: "Suno",
            baseUrl: import.meta.env.VITE_SUNO_BASE_URL || "",
            apiKey: import.meta.env.VITE_SUNO_API_KEY || "",
            apiFormat: "openai",
            models: [{ name: "suno-music", capability: "audio", script: SUNO_MUSIC_SCRIPT }],
        },
    ],
    ...
    models: ["default::gpt-image-2", "suno::suno-music"],
    quality: "auto",
    size: "1:1",
    background: "",
    count: "1",
