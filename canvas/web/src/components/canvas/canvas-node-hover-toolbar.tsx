...
type CanvasNodeHoverToolbarProps = {
    ...
    onSaveToNautilus?: (node: CanvasNodeData) => void;
...
export function CanvasNodeHoverToolbar({
    ...
    const nautilusAssetId = (node.metadata as { assetId?: string } | undefined)?.assetId;
    const canSaveToNautilus = !nautilusAssetId && (hasImage || hasVideo || hasAudio) && Boolean(node.metadata?.storageKey || node.metadata?.images?.length || node.metadata?.content);
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(canSaveToNautilus ? [{ id: "saveNautilus", title: "保存到 Nautilus 素材库", label: "入 Nautilus", icon: <Upload className="size-4" />, onClick: () => onSaveToNautilus(node) }] : []),
